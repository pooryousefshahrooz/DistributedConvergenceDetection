    ...
    while(temp != NULL )
      {
        ...
        zlog_debug("we are goign to compare %s and %s ",temp-> prefix,in_prefix);
        zlog_debug("we are goign to compare %ld and %ld ",temp-> E_id,in_E_id);
...
struct received_prefix * get_from_received_prefix(struct received_prefix ** head_ref, char* in_prefix, struct peer* in_peer){
    ...
    while(temp != NULL){
      ...
      // && ((temp -> peer_received_from -> host == in_peer -> host))
        if(strcmp(temp -> prefix_received  ,in_prefix) == 0){
            result = temp;
            return result;
