                               ...
                               struct bgp_node *rn, afi_t afi, safi_t safi)
{
 ...
 // zlog_debug ("we are in bgp_process_announce_selected for %s  after irst update is deferred until ORF", peer->host);
  ...
  switch (bgp_node_table (rn)->type)
    ...
    zlog_debug ("this is if and I will not send any fizzle to peer->host %s and selected->peer->host  %s  and prefix %s", peer->host,selected->peer->host,inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN));
    }
  else
  {
    zlog_debug ("this is the else and  peer->host %s and selected->peer->host  %s  and prefix %s", peer->host,selected->peer->host,inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN));
    struct prefix_neighbour_pair * prefix_best_peer = (struct prefix_neighbour_pair *) malloc(sizeof(struct prefix_neighbour_pair));
    prefix_best_peer = get_from_prefix_neighbour_pair(&(a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair), inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN));
    if (prefix_best_peer!=NULL)
    {
      if (prefix_best_peer->val_peer->host != peer->host)
        {
            ...
            the_time_stamp_e_id_containing_peer_ds = get_from_received_prefix(&(a_peer_for_maintating_head_of_data_structure -> received_prefix),inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN) , peer );
            ...
            // if (the_time_stamp_e_id_containing_peer_ds!=NULL)

            // //bgp_fizzle_send (peer,the_time_stamp_e_id_containing_peer_ds -> event_id,the_time_stamp_e_id_containing_peer_ds -> time_stamp,selected->peer->as,inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),the_time_stamp_e_id_containing_peer_ds->received_E_owner_id);
            // else
            //   zlog_debug ("___________we got a big errorrrrr________");     
          }
      else
        zlog_debug ("I will not send fizzle to the best till i receive fizzle from sent update and the best peer %s and going to send peer %s",prefix_best_peer->val_peer->host ,peer->host);
    }
    else
    {

       zlog_debug ("___________________*******... we will send FIZZLEE message to for prefix %s and the peer->host is %s",inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),peer->host);

          struct received_prefix * the_time_stamp_e_id_containing_peer_ds3 = (struct received_prefix *) malloc(sizeof(struct received_prefix));


            the_time_stamp_e_id_containing_peer_ds3 = get_from_received_prefix(&(a_peer_for_maintating_head_of_data_structure -> received_prefix),inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN) , peer );
              
            //delete_from_sent(&(a_peer_for_maintating_head_of_data_structure -> sent),"123", peer, 8888, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN) );
            zlog_debug ("___________________*******... lets send FIZZLEE message to for prefix %s and the peer->host is %s",inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),peer->host);

           // if (the_time_stamp_e_id_containing_peer_ds3!=NULL)

            //bgp_fizzle_send (peer,the_time_stamp_e_id_containing_peer_ds3 -> event_id,the_time_stamp_e_id_containing_peer_ds3 -> time_stamp,selected->peer->as,inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),the_time_stamp_e_id_containing_peer_ds3->received_E_owner_id);
      
        //send to all incommings
