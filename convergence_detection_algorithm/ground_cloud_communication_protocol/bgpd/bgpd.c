...
/* Allocate new peer for maintating head of data structure */





// struct peer_list* head_for_peer_list;
//add_to_peer_list(&(test_peer_list), temp5);



// struct peer *head_of_peer_converged_structure;
// /* Allocate new peer for maintating head of data structure */
// head_of_peer_converged_structure = XCALLOC (MTYPE_BGP_PEER, sizeof (struct peer));
// head_of_peer_converged_structure -> converged = NULL;


// struct peer *head_of_prefix_received_structure;
// /* Allocate new peer for maintating head of data structure */
// head_of_prefix_received_structure = XCALLOC (MTYPE_BGP_PEER, sizeof (struct peer));



// struct peer *head_of_prefix_neighbor_pair_structure;
// /* Allocate new peer for maintating head of data structure */
// head_of_prefix_neighbor_pair_structure = XCALLOC (MTYPE_BGP_PEER, sizeof (struct peer));

// struct peer *head_of_prefix_neighbors_structure;
// /* Allocate new peer for maintating head of data structure */
// head_of_prefix_neighbors_structure = XCALLOC (MTYPE_BGP_PEER, sizeof (struct peer));
...
void insert_in_converged(struct converged ** head_ref, uint32_t in_event_id){
...
void set_converged_yet_true(struct converged ** head_ref, uint32_t in_event_id){
    int found = 0;
    struct converged * temp = (*head_ref);
    while(temp != NULL){
        if(temp -> event_id == in_event_id){
...
int get_converged_yet_value(struct converged ** head_ref, uint32_t in_event_id){
    struct converged * temp = (*head_ref);
    while(temp != NULL){
        if(temp -> event_id == in_event_id){
...
void insert(struct Node** head_ref, uint32_t in_event_id, struct peer* in_peer_list)
{
...
struct Node* getNode(struct Node** head_ref, uint32_t in_event_id){
    struct Node* temp = (*head_ref);
...
void delete_node(struct Node** head_ref, uint32_t in_event_id){
    struct Node* temp = (*head_ref);
    while(temp != NULL){
...
void print_cause(struct cause** head_ref){
    ...
    while(temp != NULL){
        ...
        zlog_debug("this is the new_timestamp: %s", temp-> new_timestamp);
...
void add_to_sent(struct sent** head_ref, char *in_time_stamp, struct peer* in_neighbour, uint32_t in_E_id, char * in_prefix){
...
void delete_from_sent(struct sent** head_ref, struct peer* in_neighbour, uint32_t in_E_id, char * in_prefix ){
    ...
    while(temp != NULL )
      {
        zlog_debug("we are goign to compare local as %ld and %ld ",temp-> neighbour->local_as,in_neighbour->local_as);
        zlog_debug("we are goign to compare prefix %s and %s ",temp-> prefix,in_prefix);
        zlog_debug("we are goign to compare E_id %ld and %ld ",temp-> E_id,in_E_id);

      if ((temp-> neighbour->local_as==in_neighbour->local_as) && (strcmp(temp-> prefix ,in_prefix)==0))//&& (strcmp(temp -> timestamp ,in_time_stamp)==0)
      {
...
bool check_if_sent_is_empty(struct sent** head_ref,uint32_t  in_E_id, char * in_prefix ){
    struct sent * temp = (*head_ref);
    ...
    while(temp != NULL && temp -> E_id != in_E_id)
    {
        prev = temp;
        temp = temp -> next;
    }
...
void print_sent(struct sent ** head_ref){
    ...
    while(temp != NULL)
    {
        ...
        zlog_debug("this is the E_id %d", temp -> E_id);
        zlog_debug("this is the prefix %s", temp -> prefix);
        zlog_debug("this is the neighbour local_as %d", temp -> neighbour -> as);
        count = count + 1;
        temp = temp -> next;
    }
...
void add_to_received_prefix(struct received_prefix** head_ref,char *in_prefix, char *in_time_stamp, struct peer* in_peer, u_int32_t in_event_id, u_int32_t E_owner_id ){
    ...
    new_node5 -> peer_received_from = in_peer;
    new_node5 -> event_id = in_event_id;
    new_node5 -> received_E_owner_id = E_owner_id;

    new_node5 -> next = (*head_ref);
    (*head_ref) = new_node5;
}
struct received_prefix * get_from_received_prefix(struct received_prefix ** head_ref, char* in_prefix, struct peer* in_peer){
    ...
    while(temp != NULL){
      //char my_prefix2;
       //strncpy(my_prefix2, in_prefix, 100);
      // && ((temp -> peer_received_from -> host == in_peer -> host))
        if((strcmp(temp -> prefix_received  ,in_prefix) == 0)&& (temp -> peer_received_from -> host == in_peer -> host))
        {
            result = temp;
            return result;
            //zlog_debug("********.................**** this %s and this %s are same  ******............******",temp -> prefix_received,in_prefix);

            break;
        }else{

            //zlog_debug("********.................**** this %s and this %s are not same  ******............******",temp -> prefix_received,in_prefix);
...
void print_received_prefix(struct received_prefix ** head_ref){
    ...
    while(temp != NULL){
        ...
        zlog_debug(" this is the before time_stamp");
        zlog_debug(" this is the time_stamp %s", temp -> time_stamp);
        zlog_debug(" this is the local_as of peer that we received from %ld", temp -> peer_received_from -> as);
        zlog_debug("this is the event_id %ld", temp -> event_id);
        count = count+1;
        temp = temp -> next;
    }
...
void add_to_senders(struct senders_of_a_prefix ** head_ref, struct peer_list* in_peer_list){
    struct senders_of_a_prefix * new_node3 = (struct senders_of_a_prefix *) malloc(sizeof(struct senders_of_a_prefix));
...
void add_to_senders_of_a_prefix(struct senders_of_a_prefix ** head_ref, char *in_prefix, struct peer_list* in_peer_list){
    struct senders_of_a_prefix * new_node3 = (struct senders_of_a_prefix *) malloc(sizeof(struct senders_of_a_prefix));
...
void remove_senders(struct senders_of_a_prefix** head_ref){
    struct senders_of_a_prefix * temp = (*head_ref);
    struct senders_of_a_prefix * result = NULL;
    while(temp != NULL){
       // zlog_debug("we are going to compare %s and %s to get the peer list",temp -> key_prefix,in_prefix );
        //if(strcmp (temp -> key_prefix,in_prefix)==0){
            *head_ref = temp->next;   // Changed head
            free(temp);               // free old head
...
void remove_senders_of_a_prefix(struct senders_of_a_prefix** head_ref, char *in_prefix){
    struct senders_of_a_prefix * temp = (*head_ref);
    struct senders_of_a_prefix * result = NULL;
    while(temp != NULL){
       // zlog_debug("we are going to compare %s and %s to get the peer list",temp -> key_prefix,in_prefix );
...
void remove_neighbours_of_a_prefix(struct neighbours_of_a_prefix** head_ref, char *in_prefix){
...
struct senders_of_a_prefix * get_from_senders(struct senders_of_a_prefix** head_ref)
{
    struct senders_of_a_prefix * temp = (*head_ref);
    struct senders_of_a_prefix * result = NULL;
    return temp;
...
struct senders_of_a_prefix * get_from_senders_of_a_prefix(struct senders_of_a_prefix** head_ref, char *in_prefix){
    struct senders_of_a_prefix * temp = (*head_ref);
    struct senders_of_a_prefix * result = NULL;
    while(temp != NULL){
       //zlog_debug("we are going to compare %s and %s to get the peer list",temp -> key_prefix,in_prefix );
...
void print_senders(struct senders_of_a_prefix ** head_ref){
    struct senders_of_a_prefix * temp = (*head_ref);
    int count = 0;
    zlog_debug("******* going to print senders of a prefix list ********");
    while(temp != NULL){
...
void print_senders_of_a_prefix(struct senders_of_a_prefix ** head_ref){
    struct senders_of_a_prefix * temp = (*head_ref);
    int count = 0;
    zlog_debug("******* going to print senders of a prefix list ********");
    while(temp != NULL){
    ...
    zlog_debug("print senders of a prefix ends here ");
    return;
}
...
void delete_from_prefix_neighbour_pair(struct prefix_neighbour_pair ** head_ref, char * in_prefix,struct peer * in_peer)
...
struct prefix_neighbour_pair * temp = (*head_ref);
    ...
    while(temp != NULL)
    {
        //char my_prefix4[] =temp -> my_prefix;
         //my_prefix4 = temp -> my_prefix;
        if( (strcmp(temp -> my_prefix, in_prefix)==0) && in_peer->local_as== temp->val_peer->local_as)
        {
          //zlog_debug("********.................**** this %s and this %s are same  ******............******",in_prefix,temp -> my_prefix);
        *head_ref = temp->next;   // Changed head
        free(temp); 
            break;
        }else{
          //zlog_debug("********.................**** this %s and this %s are not same  ******............******",in_prefix,temp -> my_prefix);
            temp = temp -> next;
        }
    }
    return;
...
{
  zlog_debug("********........lets get the pair of this prefix %s........**** ",in_prefix);
    struct prefix_neighbour_pair * temp = (*head_ref);
    struct prefix_neighbour_pair * result = NULL;
    while(temp != NULL)
    {
        //char my_prefix4[] =temp -> my_prefix;
         //my_prefix4 = temp -> my_prefix;
        if( (strcmp(temp -> my_prefix, in_prefix)==0))
        {
          zlog_debug("********.................**** this %s and this %s are same  ******............******",in_prefix,temp -> my_prefix);
...
void print_prefix_neighbour_pair(struct prefix_neighbour_pair ** head_ref){
    ...
    while(temp != NULL){
        ...
        zlog_debug("this is the prefix %s ",temp -> my_prefix);
        zlog_debug("this is the neighbour's host value %s",  temp -> val_peer -> host);
        count = count + 1;
        temp = temp -> next;
    }
...
peer_new (struct bgp *bgp)
{
   ...
   //a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix = NULL;
   //a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair = NULL;
//   a_peer_for_maintating_head_of_data_structure -> cause = NULL;
// a_peer_for_maintating_head_of_data_structure -> converged = NULL;
// a_peer_for_maintating_head_of_data_structure -> sent = NULL;
// a_peer_for_maintating_head_of_data_structure -> received_prefix = NULL;
// a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix = NULL;
// a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair = NULL;


//head_for_peer_list = NULL;

    // a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair = NULL;
    // struct peer * check_peer2 = (struct peer *) malloc(sizeof(struct peer));
    // check_peer2 -> local_as = 4545;
    // check_peer2 -> host = "host 4545";
    // add_to_prefix_neighbour_pair(&(a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair ), "44.33.22.11", check_peer2);
  ...
  a_peer_for_maintating_head_of_data_structure = XCALLOC (MTYPE_BGP_PEER, sizeof (struct peer));
  //a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair = NULL;
  ...
  //** Set Data structures particular to Convergence detection*/
    ...
    //add_to_received_prefix(&(peer -> received_prefix), "25.25.25.25", "123,45", temp, 5155);
