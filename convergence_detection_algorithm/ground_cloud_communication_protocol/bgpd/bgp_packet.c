...
void
bgp_convergence_send (struct peer *peer,uint32_t *passed_root_cause_event_id,char *our_target_prefix)
{
  ...
  char sending_prefix_value[50];
  strcpy(sending_prefix_value , our_target_prefix);

  //zlog_debug ("We are going to make a BGP_MSG_CONVERGENCE message to send to %s",peer->host);
  //zlog_debug ("We are going to send a BGP_MSG_CONVERGENCE message to  %s and the parameters are E_id is %ld , and prefix %s ",peer->host,passed_root_cause_event_id,our_target_prefix);
  //zlog_debug(" lets convert prefix (%s)to four numbers to send %s", prefix);
  char my_delim[]= ".";
  char *my_ptr = strtok(sending_prefix_value, my_delim);
  int my_ip_array[4];
  int i=0;
 ...
 stream_putl (s, my_ip_array[3]);
  ...
  stream_reset (s);
  stream_reset (snlri);
  //zlog_debug ("We are at the returning packet point for sending convergence message %s for prefix %s  ", ".....---------..................",our_target_prefix,sending_prefix_value);
...
/* Make bgp_event_data_send packet and send it to the peer. */

void
bgp_event_data_send (struct peer *peer,uint32_t *passed_root_cause_event_id,uint32_t *passed_root_cause_event_owner_router_id,char *passed_prefix, uint32_t *E_owner_id)
{


zlog_debug ("outgoing bgp_event_data_send message to %s ",peer->host);



zlog_debug ("We are going to send a BGP_MSG_EVENT_DATA message to %s",peer->host);
zlog_debug ("We are going to send a BGP_MSG_EVENT_DATA message to %ld",passed_root_cause_event_id);
zlog_debug ("We are going to send a BGP_MSG_EVENT_DATA message to %ld",passed_root_cause_event_owner_router_id);
zlog_debug ("We are going to send a BGP_MSG_EVENT_DATA message to %s",passed_prefix);
zlog_debug ("We are going to send a BGP_MSG_EVENT_DATA message to %ld",E_owner_id);

//zlog_debug ("We are going to send a BGP_MSG_FIZZLE message to %s and the parameters are time_stamp is %s,E_id is %ld , and router_owner is %ld ",peer->host,passed_root_cause_event_id,E_owner_id);
...
//       p->prefixlen);
  ...
  stream_reset (snlri);
    ...
    zlog_debug ("%s We are before BGP_ADV_FIFO_HEAD", ".......................");
  ...
  s = stream_new (BGP_MAX_PACKET_SIZE);
  bgp_packet_set_marker (s, BGP_MSG_EVENT_DATA);
  ...
  stream_putl (s, my_ip_array[3]);
     ...
     */
    mpattr_pos = stream_get_endp(s);
    zlog_debug ("%s We are after  stream_get_endp", ".....---------..................");
  ...
  /////
    ...
    stream_reset (s);
    stream_reset (snlri);
    //zlog_debug ("%s We are at the returning packet point for sending BGP_MSG_EVENT_DATA", ".....---------..................");
...
/* Make BGP_NEW_FIB_ENTRY packet and send it to the peer. */

void
 send_new_FIB_entries_to_ground(struct peer *peer,char *target_prefix,uint32_t *next_hop_of_prefix)
{



char sending_prefix_value[50];
//sending_prefix_value = passed_prefix;
strcpy(sending_prefix_value , target_prefix);



zlog_debug ("outgoing new FIB entry message to %s for prefix %s ",peer->host,target_prefix);
// zlog_debug ("We are going to send a BGP_MSG_FIZZLE message to %s",peer->host);
// zlog_debug ("We are going to send a BGP_MSG_FIZZLE message to %ld",passed_root_cause_event_id);
// zlog_debug ("We are going to send a BGP_MSG_FIZZLE message to %ld",passed_root_cause_event_owner_router_id);
// zlog_debug ("We are going to send a BGP_MSG_FIZZLE message to %s",passed_prefix);
// zlog_debug ("We are going to send a BGP_MSG_FIZZLE message to %ld",E_owner_id);
...
//       p->prefixlen);
    ...
    char *my_ptr = strtok(target_prefix, my_delim);
    int my_ip_array[4];
    int i=0;
  ...
  s = stream_new (BGP_MAX_PACKET_SIZE);
  bgp_packet_set_marker (s, BGP_MSG_FIB_ENTRY);
    ...
    /* 2: Write timestamp */
   stream_putl (s, next_hop_of_prefix);
  ...
  /////
    ...
    stream_reset (s);
    stream_reset (snlri);
    //zlog_debug ("%s We are at the returning packet point for sending FIB entry for prefix %s and the saved one %s", ".....---------..................",target_prefix,sending_prefix_value );
    strcpy(target_prefix,sending_prefix_value );
    //zlog_debug ("%s We are at the returning packet point for sending FIB entry for prefix %s and the saved one %s", ".....---------..................",target_prefix,sending_prefix_value );
...
void
bgp_fizzle_send (struct peer *peer,uint32_t *passed_root_cause_event_id,uint32_t *passed_root_cause_event_owner_router_id,char *passed_prefix, uint32_t *E_owner_id)
{

char sending_prefix_value[50];
//sending_prefix_value = passed_prefix;
strcpy(sending_prefix_value , passed_prefix);


zlog_debug ("outgoing FIZZLE message to %s ",peer->host);
...
//       p->prefixlen);
  ...
  /////
    ...
    stream_reset (s);
    stream_reset (snlri);
    //zlog_debug ("%s We are at the returning packet point for sending FIZZLE for prefix %s , %s ", ".....---------..................",sending_prefix_value,passed_prefix);
...
bgp_update_packet (struct peer *peer, afi_t afi, safi_t safi)
{
  ...
  // if (peer->as / 10 == peer->local_as) 
  //   {
  //   zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!!!****!!!!!!this is the my ground instance and I will not announce any prefix to him %ld", peer->as);
  //     //return NULL;
  //   zlog_debug ("this is the peer->as / 10 %ld , peer->local_as  %ld for preventing sending to ground", peer->as / 10 , peer->local_as);
  //   return NULL;
  //   }

  //   if (peer->as * 10 == peer->local_as) 
  //   {
  //   zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!!!****!!!!!!this is the my cloud instance and I will not announce any prefix to him %ld", peer->as);
  //     //return NULL;
  //       zlog_debug ("this is peer->as * 10 %ld ,peer->local_as  %ld for preventing sending to cloud ", peer->as * 10 , peer->local_as);
  //       return NULL;
  //   }
  ...
  while (adv)
    ...
    //zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!******!!!!! this is %s prefix value in while adv",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ) );

//zlog_debug("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++This is next hop of this prefix  %s %s ",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),adv->baa->attr->nexthop);

    //zlog_debug("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++This is next hop of this prefix  %s %ld ",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),adv->baa->attr->origin);
...
/*                   we are going to send back to those senders which their as number is not in this prefix's first ASPATH */
    struct senders_of_a_prefix * the_senders_of_the_prefix = (struct senders_of_a_prefix *) malloc (sizeof(struct senders_of_a_prefix));

    the_senders_of_the_prefix = get_from_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix),inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
    if (the_senders_of_the_prefix != NULL)
    {
        //zlog_debug("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++This is  ASPATH of this prefix  %s %sdddd ",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),adv->baa->attr->aspath->str);
        char my_delim[]= " ";
        char *my_ptr = strtok(adv->baa->attr->aspath->str, my_delim);
        ...
        //zlog_debug(" ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ this is the first as in the next steps of a ASPATH %d ", my_aspath_array[0]);
        print_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix));
        struct peer_list * my_temp = the_senders_of_the_prefix -> peer_list ;
        while(my_temp != NULL)
        {
            //zlog_debug("this is the host of the peer %s and the as number %ld we have received this prefix %s from", my_temp -> peer -> host,my_temp->peer->as,inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ) );
            char local_as_char[50]; 
            sprintf(local_as_char, "%u", my_temp->peer->as); 
            char next_as[50]; 
            sprintf(next_as, "%u", my_aspath_array[0]); 
            //zlog_debug("we are going to compare %s and  %s", local_as_char,next_as);

            if (strcmp(local_as_char , next_as)==0)
            {
              struct received_prefix * res_rec_pref = (struct received_prefix *) malloc(sizeof(struct received_prefix));
              res_rec_pref = get_from_received_prefix(&(a_peer_for_maintating_head_of_data_structure -> received_prefix),inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), my_temp->peer);
              struct timeval te2; 
              gettimeofday(&te2, NULL); // get current time
              //long gen_time_stamp = te2.tv_sec*1000LL + te2.tv_usec/1000; // calculate milliseconds
              //uint32_t generated_time_stamp = te.tv_sec*1000LL + te.tv_usec/1000; // calculate milliseconds

              // uint32_t generated_time_stamp = 100 + rand() / (RAND_MAX / (1000 - 100 + 1) + 1);
              uint32_t generated_time_stamp = te2.tv_sec*1000LL + te2.tv_usec/1000; // calculate milliseconds

              char my_last_four[50];
              sprintf(my_last_four, "%u", generated_time_stamp); 


              int my_picked_time_stamp_from_local_time = generated_time_stamp %1000000;
              char my_last_six[50];
              sprintf(my_last_six, "%u", my_picked_time_stamp_from_local_time); 
              //zlog_debug("*********  my last six %s as my time stamp *********",my_last_six);

              // uint32_t time_stamp_generated = 100 + rand() / (RAND_MAX / (1000 - 100 + 1) + 1);
              // time_stamp_generated = time_stamp_generated +peer->local_as;
              my_picked_time_stamp_from_local_time = my_picked_time_stamp_from_local_time+peer->local_as;



              char local_time_stamp[50]; 
              sprintf(local_time_stamp, "%u", my_picked_time_stamp_from_local_time); 
              if (avatar)
              if (avatar->as != peer->as)
              {
              addcause(&(a_peer_for_maintating_head_of_data_structure -> cause),local_time_stamp,"CBGPMSG",res_rec_pref -> event_id, peer->as,inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), "1 2 3", res_rec_pref -> time_stamp,res_rec_pref->peer_received_from);

              add_to_sent(&(a_peer_for_maintating_head_of_data_structure -> sent),local_time_stamp,peer,res_rec_pref -> event_id , inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
              }
              else  
               {
              zlog_debug("...............................................................................Lets not send update to this peer  %s as this is my avatar %s  ",peer->host);
              //print_sent(&(a_peer_for_maintating_head_of_data_structure -> sent));

                    }
              
              //zlog_debug("check: my_time_stamp value is %s  ",local_time_stamp);
              ...
              //zlog_debug("**************************   the cause of %s with time stamp %s sent to %s is time stamp  %s received from %s ",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),local_time_stamp,peer->host,local_time_stamp,res_rec_pref->peer_received_from->host);
              ...
              //zlog_debug("**************************   We are sending %s with time stamp %s  to %s **********",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),local_time_stamp,peer->host);

              //zlog_debug("check: my_time_stamp value is %s  ",my_time_stamp);

              stream_putl (s, my_picked_time_stamp_from_local_time);
                /* 2: Write root cause event ID */
    ...
    {
        zlog_debug (" We have not received prefix %s from any neighbor!!!", inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
      ...
      uint32_t my_time_stamp_generated = te.tv_sec*1000LL + te.tv_usec/1000; // calculate milliseconds

      my_time_stamp_generated = my_time_stamp_generated %1000000;
      my_time_stamp_generated = my_time_stamp_generated +peer->local_as;
      char local_time_stamp_char[50];
      sprintf(local_time_stamp_char, "%u", my_time_stamp_generated); 
      //zlog_debug("*********  my last six %s *********",local_time_stamp_char);

      // uint32_t time_stamp_generated = 100 + rand() / (RAND_MAX / (1000 - 100 + 1) + 1);
      // time_stamp_generated = time_stamp_generated +peer->local_as;
      // time_stamp_generated = my_six_digits;
     ...
     uint32_t generated_root_cause_id = 100 + rand() / (RAND_MAX / (10000 - 100 + 1) + 1);
     generated_root_cause_id = generated_root_cause_id + peer->local_as;


      //zlog_debug("Time Stamp and E_ID  and owner id is are %s and  %ld  and %ld ********  for prefix %s and in_neighbor is %s",local_time_stamp_char,generated_root_cause_id,peer->local_as,inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),peer->host);
    ...
    //   zlog_debug("******************the cause of %s with time stamp %s sent to %s is GRCMSG generated by  %ld ",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),char_milliseconds,peer->host,peer->as);
      ...
      if (avatar->as != peer->as)
      {
      //zlog_debug("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! adding to sent for prefix %s",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
      

      add_to_sent(&(a_peer_for_maintating_head_of_data_structure -> sent),local_time_stamp_char,peer, generated_root_cause_id, inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
      //print_sent(&(a_peer_for_maintating_head_of_data_structure -> sent));

      addcause(&(a_peer_for_maintating_head_of_data_structure -> cause),local_time_stamp_char,in_received_time_stamp,generated_root_cause_id, peer->local_as,inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), "1 2 3", in_received_time_stamp,peer);
     }
     else
     {
        zlog_debug("...............................................................................Lets not send update to this peer  %s as this is my avatar %s  ",peer->host);
      ...
      //zlog_debug("******************");
      stream_putl (s, my_time_stamp_generated);
        /* 2: Write root cause event ID */
      stream_putl (s, generated_root_cause_id);
      root_cause_event_owner = peer->local_as;
      stream_putl (s, root_cause_event_owner);
    }
    /*               we finished sending fizzle to senders their as number is not in first ASPATH of this prefix */
      ...
      // struct neighbours_of_a_prefix * the_neighbours_of_the_prefix = (struct neighbours_of_a_prefix *) malloc (sizeof(struct neighbours_of_a_prefix));
      // the_neighbours_of_the_prefix = get_from_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix),inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
      //working on point
    struct peer_list* test_appending_peer_list = NULL;
  ...
  struct neighbours_of_a_prefix * the_neighbours_of_the_prefix = (struct neighbours_of_a_prefix *) malloc (sizeof(struct neighbours_of_a_prefix));
  the_neighbours_of_the_prefix = get_from_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix),inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
    if (the_neighbours_of_the_prefix != NULL)
    {
        if (avatar!= NULL)
        if (peer->as != avatar->as)
        {
        ...
        remove_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix), inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
        //zlog_debug (" after removing to the list and neighbors of the prefix");
        //print_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix));

        struct peer_list * my_temp = the_neighbours_of_the_prefix -> peer_list ;
        //zlog_debug("********** going to print the peer list ********");
        while(my_temp != NULL)
        {
           // zlog_debug("this is the host of the peer %s we are sending this prefix %s to", my_temp -> peer -> host,inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ) );
            //bgp_convergence_send (my_temp -> peer,rec_root_cause_event_id, received_prefix);
            add_to_peer_list(&(test_appending_peer_list), my_temp -> peer);
            my_temp = my_temp -> next;
         }

        //zlog_debug("Lets add this new peer %s to the list too as a peer we are sending prefix %s to it",peer->host,inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));

          add_to_peer_list(&(test_appending_peer_list),  peer);
        
       // zlog_debug("We added to the list");
        add_to_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix), inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), test_appending_peer_list);
    ...
    else
    {
        ...
        struct peer_list* test_appending_peer_list2 = NULL;
        add_to_peer_list(&(test_appending_peer_list2),  peer);
       // zlog_debug("We added just this one to the list");
        add_to_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix), inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), test_appending_peer_list2);
        //print_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix));
    }
  ...
  //   print_received_prefix(&(a_peer_for_maintating_head_of_data_structure -> received_prefix));

  //   if (strcmp("Static announcement", best_received_neighbor-> val_peer->host)!=0)
  // {
  //   zlog_debug ("For prefix %s we have the best peer %s and we are going to get the time stamp from function",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ),best_received_neighbor-> val_peer->host);

  //   struct received_prefix * res_rec_pref = (struct received_prefix *) malloc(sizeof(struct received_prefix));
    
  //   res_rec_pref = get_from_received_prefix(&(a_peer_for_maintating_head_of_data_structure -> received_prefix), inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), best_received_neighbor-> val_peer);
  
  //   char *my_time_stamp[50];
  //   strncpy(my_time_stamp, res_rec_pref -> time_stamp, 50);

  //   if (res_rec_pref!= NULL)// && best_received_neighbor -> val_peer -> host ==res_rec_pref->peer_received_from->host
  //     {
  //     // generate time stamp
  //     struct timeval te2; 
  //     gettimeofday(&te2, NULL); // get current time
  ...
  //   }
  //       else{
  //         zlog_debug("We have Static announcement as best peer for this prefix %s",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
          
  //         we_have_received_prefix = false;
  //       }
  // }
  //       else
  //       {
  ...
  // else{// we have not received any update so we are the owner of event,create GRCMSG
  ...
  //     char my_last_four[50];
  //     sprintf(my_last_four, "%u", my_time_stamp_generated); 
  //     zlog_debug("********* my whole digits %s *********",my_last_four);


  //     int my_six_digits = my_time_stamp_generated %1000000;
  //     char my_last_six[50];
  //     sprintf(my_last_six, "%u", my_six_digits); 
  //     zlog_debug("*********  my last six %s *********",my_last_six);
...
//zlog_debug("The working on prefix is %s ",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
   ...
   //stream_putl (s, peer->router-id);
    ...
    stream_putw (s, 0);
      ...
      if (afi == AFI_IP && safi == SAFI_UNICAST)
      {
        ...
        //zlog_debug("????????????????????????????????????????????????????????????????Are we in this loop ever? ");
...
bgp_withdraw_packet (struct peer *peer, afi_t afi, safi_t safi)
{
  ...
  while ((adv = BGP_ADV_FIFO_HEAD (&peer->sync[afi][safi]->withdraw)) != NULL)
	...
	first_time = 0;
      ...
      if (afi == AFI_IP && safi == SAFI_UNICAST)
      {
          ...
          zlog_debug ("++++++++++++++++++++++++ we are going to  stream_put_prefix at %ld point",stream_get_endp (s));
    ...
    zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!!! we are in else of  stream_put_prefix ");
	  ...
	  if (first_time)
	    {
	      attrlen_pos = stream_get_endp (s);
        zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!!! this is the  attrlen_pos %ld",attrlen_pos);

	      /* total attr length = 0 for now. reevaluate later */
	      stream_putw (s, 0);
        zlog_debug ("++++++++++++++++++++++++ we putted zero");

	      mp_start = stream_get_endp (s);

        zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!!! this is mp_start value %ld",mp_start);

	      mplen_pos = bgp_packet_mpunreach_start(s, afi, safi);
        zlog_debug ("!!!!!!!!!!!!!!!!!!!!!!!!!! this is mplen_pos value %ld",mplen_pos);

	    }
  zlog_debug ("++++++++++++++++++++++++ we are putting prefix in strem");
  ...
  if (! stream_empty (s))
	...
	{
	  unfeasible_len
	    = stream_get_endp (s) - BGP_HEADER_SIZE - BGP_UNFEASIBLE_LEN-12;
    zlog_debug ("++++++++++++++++++++++++ if strem not empty and we are adding unfeasible_len which is %ld  at the point %ld of strem",unfeasible_len,BGP_HEADER_SIZE);

	  stream_putw_at (s, BGP_HEADER_SIZE+12, unfeasible_len);
    zlog_debug ("++++++++++++++++++++++++ if strem not empty and we are adding a zero at the point of %ld",stream_get_endp (s));

	  stream_putw (s, 0);
	}
      else
	{
    zlog_debug ("++++++++++++++++++++++++ if strem is empty  and we are adding mplen_pos value to srtrem");
    ...
    zlog_debug ("!!!!!!!!!!!!!!!!! this is total_attr_len value %ld ",total_attr_len);

    zlog_debug ("++++++++++++++++++++++++ we are adding attr_len and pos in stream here");
...
bgp_write_packet (struct peer *peer)
{
 ...
 // zlog_debug ("we are in bgp_write_packet");
  afi_t afi;
  safi_t safi;
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
	...
	if (adv)
	  {
      zlog_debug ("we are before calling bgp_withdraw_packet");
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
	...
	if (adv)
	  {
      if(peer->as / 10 != peer->local_as || peer->as * 10 != peer->local_as)
      {
            if (adv->binfo && (adv->binfo->uptime < peer->synctime || 1==1))
	      {
...
bgp_write (struct thread *thread)
{
  ...
  if (!s)
  {
     // zlog_debug ("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++we are in bgp_write and we have nothing to send!!!!!!!!!!! to %s",peer->host);
...
//zlog_debug ("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++we are in bgp_write and we it seems we have something to send!!!!!!!!!!! to %s ",peer->host);
  ...
  if (bgp_write_proceed (peer))
    BGP_WRITE_ON (peer->t_write, bgp_write, peer->fd);
  //else
      //zlog_debug ("we are in else of bgp_write_proceed ");
...
zlog_debug ("incomming CONVERGENCE message from  %s  ",peer->host);
  ...
  if (get_converged_yet_value(&(a_peer_for_maintating_head_of_data_structure->converged), rec_root_cause_event_id) != 1)
      {
        //zlog_debug("this is the peer's converged event_id %ld and we need to send convergence message to others", get_converged_yet_value(&(a_peer_for_maintating_head_of_data_structure->converged), rec_root_cause_event_id));
        set_converged_yet_true(&(a_peer_for_maintating_head_of_data_structure -> converged), rec_root_cause_event_id);
        zlog_debug ("................................********  STABLE STATE DETECTION of %ld received from %s ********............................",rec_root_cause_event_id,peer->host);
        ...
        // zlog_debug ("This is %s, %ld the received prefix0 from bgp_convergence_receive function ", received_prefix,rec_prefix_part_0);
        // zlog_debug ("This is %s , %ld the received prefix1 from bgp_convergence_receive function ", char_received_prefix2,rec_prefix_part_1);
        // zlog_debug ("This is %s , %ld the received prefix2 from bgp_convergence_receive function ", char_received_prefix3,rec_prefix_part_2);
        // zlog_debug ("This is %s , %ld the received prefix3 from bgp_convergence_receive function ", char_received_prefix4,rec_prefix_part_3);
        ...
        struct prefix_neighbour_pair * prefix_best_peer = (struct prefix_neighbour_pair *) malloc(sizeof(struct prefix_neighbour_pair));
        prefix_best_peer = get_from_prefix_neighbour_pair(&(a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair), received_prefix);
        if (prefix_best_peer!=NULL)
        {
          if (avatar != NULL)
          {
          zlog_debug ("................................********  we are going to send %ld as next hop of %s ********............................",prefix_best_peer->val_peer->as,received_prefix);
          send_new_FIB_entries_to_ground(avatar,received_prefix,prefix_best_peer->val_peer->as);
          } 
          else
            zlog_debug ("BIG error as this means we do not have a avatar to send this prefix %s's next hop which is %ld",received_prefix,prefix_best_peer->val_peer->as);
        }
        else
        {
          zlog_debug ("BIG error1 as this means we have not picked any neighbor as this prefix %s's next hop",received_prefix);
          print_prefix_neighbour_pair(&(a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair));
          
          struct neighbours_of_a_prefix * the_neighbours_of_the_prefix = (struct neighbours_of_a_prefix *) malloc (sizeof(struct neighbours_of_a_prefix));
          the_neighbours_of_the_prefix = get_from_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix),received_prefix);
          if (the_neighbours_of_the_prefix == NULL)
            {
              //zlog_debug ("lets print receivd neighbors of prefix %s",inet_ntop (rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ));
            //zlog_debug ("BIG error2, lets send new FIB to %s",avatar->host );
            if (avatar)
            {
              char saving_prefix_value[50];
            //sending_prefix_value = passed_prefix;
              strcpy(saving_prefix_value , received_prefix);
              send_new_FIB_entries_to_ground(avatar,received_prefix,peer->as);
              strcpy(received_prefix,saving_prefix_value);
            }
            else
              zlog_debug ("BIG error3 as this means we do not have a avatar to send this prefix %s's next hop which is %ld",received_prefix,prefix_best_peer->val_peer->as);
          ...
          // lets senf d global convergence to all sent neighbors

      set_converged_yet_true(&(a_peer_for_maintating_head_of_data_structure -> converged), rec_root_cause_event_id);
      //zlog_debug("2this is the peer's converged event_id %ld and we need to send convergence message to others", get_converged_yet_value(&(a_peer_for_maintating_head_of_data_structure->converged), rec_root_cause_event_id));
      ...
      //zlog_debug ("2**************************** sending global convergence message for %ld  *************** and prefix is %s",rec_root_cause_event_id,received_prefix);
      ...
      test_neighbour_of_prefix = get_from_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix),received_prefix);
        if (test_neighbour_of_prefix != NULL)
        {
            ...
            while(my_temp != NULL)
            {
               //zlog_debug("2this is the host of the peer %s i will send grobal convergence", my_temp -> peer -> host);

              char saving_prefix_value4[50];
              strcpy(saving_prefix_value4 ,received_prefix );
            //zlog_debug("2in sending global convergnce before sending the prefix is %s !!!",received_prefix);
            //zlog_debug("2in sending global convergnce before sending the saved prefix is %s !!!",saving_prefix_value4);

                bgp_convergence_send (my_temp -> peer,rec_root_cause_event_id, received_prefix);
                // zlog_debug("2in sending global convergnce after sending the saved prefix is %s !!!",saving_prefix_value4);
                // zlog_debug("2in sending global convergnce after sending the actual prefix is %s !!!",received_prefix);

                strcpy(received_prefix,saving_prefix_value4);
        ...
        // else
        // {
        //     //zlog_debug("2in sending global convergnce We have not sent prefix %s to any neighbor!!!",received_prefix);
        //     //print_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix));
        // }
        ...
        zlog_debug (" We have already setted the convergence value of this Event ID  %ld to true !!!",rec_root_cause_event_id);
...
static int
announce_new_prefix_in_cloud (struct bgp *bgp, const char *ip_str, 
                afi_t afi, safi_t safi, const char *rmap, int backdoor)
{



  // return bgp_static_set (vty, vty->index, ip_str, AFI_IP6,
  //      bgp_node_safi (vty), NULL, 0);




  zlog_debug (" We are in announce_new_prefix_in_cloud ");
...
//received_prefix
  ...
  zlog_debug ("We almost announced our new prefix in cloud!:)");
...
/* Parse BGP event data packet */
static int
bgp_event_data_receive (struct peer *peer, bgp_size_t size)
{

    zlog_debug ("incomming event data from ground  from %s",peer->host);
  ...
  if (  size == 4 )
    {
    zlog_debug ("we have received a event data message but the lenght is 4 ");
  ...
  received_event_code = stream_getl (s);

    //zlog_debug ("we got time stamp time");
  char char_receiveded_time_stamp[50];
  sprintf( char_receiveded_time_stamp, "%u", received_event_code );
  ...
  strcat(received_prefix, ".");
  strcat(received_prefix, char_received_prefix4);
  zlog_debug ("%s 4.this is the received concatted prefix", received_prefix);


  zlog_debug ("We received a event data message from %s for prefix %s , time_stamp %s and E_id %s ", peer->host,received_prefix,char_receiveded_time_stamp,char_received_root_cause_event_id);
  
  if (received_event_code==1)// code number 1 is new prefix announcing
  {
    safi_t safi;
    safi = SAFI_UNICAST;
  ...
  bgp = bgp_get_default ();
    ...
    return announce_new_prefix_in_cloud (bgp, received_prefix,
         AFI_IP, safi, NULL, 0);
...
// parse BGP new FIB entry message
static int bgp_new_FIB_entry_receive(struct peer *peer , bgp_size_t size)
{


zlog_debug ("incomming new_FIB_entry message from %s",peer->host);
  ...
  if (  size == 4 )
    {
    zlog_debug ("we have received a new fib entry message but the lenght is 4 ");
  ...
  uint32_t next_hop_of_the_prefix;
  uint32_t rec_root_cause_event_id_from_fizzle;
  uint32_t rec_root_cause_event_owener_id;
  uint32_t  received_prefix1;
  uint32_t  received_prefix2;
  uint32_t  received_prefix3;
  uint32_t  received_prefix4;
  next_hop_of_the_prefix = stream_getl (s);

    //zlog_debug ("we got time stamp time");
  char char_next_hop_of_the_prefix[50];
  sprintf( char_next_hop_of_the_prefix, "%u", next_hop_of_the_prefix );
    char result18[50]; 
  ...
  zlog_debug ("**************************************************************************We received a new FIB entry message from %s for prefix %s , next hop is %ld ", peer->host,received_prefix,next_hop_of_the_prefix);
...
bgp_fizzle_receive (struct peer *peer, bgp_size_t size)
{
    ...
    //zlog_debug ("incomming FIZZLE message from %s",peer->host);
  ...
  zlog_debug ("We received a fizzle message from %s for prefix %s , time_stamp %s and E_id %s ", peer->host,received_prefix,char_receiveded_time_stamp,char_received_root_cause_event_id);
  insert_in_converged(&(a_peer_for_maintating_head_of_data_structure -> converged), rec_root_cause_event_id_from_fizzle);
  ...
  delete_from_sent(&(a_peer_for_maintating_head_of_data_structure -> sent), peer,  rec_root_cause_event_id_from_fizzle, received_prefix );
  if (check_if_sent_is_empty(&(a_peer_for_maintating_head_of_data_structure -> sent),rec_root_cause_event_id_from_fizzle , received_prefix))
    {
      ...
      {
            if (get_converged_yet_value(&(a_peer_for_maintating_head_of_data_structure->converged), rec_root_cause_event_id_from_fizzle) != 1)
            {
            zlog_debug ("................................********  STABLE STATE DETECTION of %s by myself as I am the owner(the last received fizzle was from %s) ********............................",char_received_root_cause_event_id,peer->host);
            if(avatar)
            {
              struct prefix_neighbour_pair * prefix_best_peer = (struct prefix_neighbour_pair *) malloc(sizeof(struct prefix_neighbour_pair));
              prefix_best_peer = get_from_prefix_neighbour_pair(&(a_peer_for_maintating_head_of_data_structure -> pref_neigh_pair), received_prefix);
              if (prefix_best_peer!=NULL)
              {
                ...
                char lets_save_prefix_value[50];
                //sending_prefix_value = passed_prefix;
                strcpy(lets_save_prefix_value , received_prefix);

                send_new_FIB_entries_to_ground(avatar,received_prefix,prefix_best_peer->val_peer->as);

                //zlog_debug ("................................********  we did the going to send %ld as next hop of %s ********............................",prefix_best_peer->val_peer->as,received_prefix);
                //zlog_debug ("................................********  we did again  going to send %ld as next hop of %s ********............................",prefix_best_peer->val_peer->as,lets_save_prefix_value);

                strcpy(received_prefix , lets_save_prefix_value);
                //zlog_debug ("................................********  we did again  going to send %ld as next hop of %s ********............................",prefix_best_peer->val_peer->as,received_prefix);

              }
              else
                zlog_debug ("BIG error as this means we have not picked any neighbor as this prefix %s's next hop",received_prefix);
              
            }
            else
              zlog_debug ("We do not have any ground instanse to send FIB entro for %s interesting!!!! :)",received_prefix);

            //zlog_debug("this is the peer's converged event_id %ld", get_converged_yet_value(&(a_peer_for_maintating_head_of_data_structure->converged), rec_root_cause_event_id_from_fizzle));


            set_converged_yet_true(&(a_peer_for_maintating_head_of_data_structure -> converged), rec_root_cause_event_id_from_fizzle);
            //zlog_debug("this is the peer's converged event_id %ld and we need to send convergence message to others", get_converged_yet_value(&(a_peer_for_maintating_head_of_data_structure->converged), rec_root_cause_event_id_from_fizzle));

            //zlog_debug ("**************************** it seems my sent is empty and I am the owner of event it, the owner_id is %s and my id is %ld    ***************",char_owner_id,peer->as);
            zlog_debug ("**************************** sending global convergence message for %s  *************** and prefix is %s",char_received_root_cause_event_id,received_prefix);
            // for all prefix_neighbors:
            //   send_convergence_message()
      struct neighbours_of_a_prefix * test_neighbour_of_prefix = (struct neighbours_of_a_prefix *) malloc (sizeof(struct neighbours_of_a_prefix));
      test_neighbour_of_prefix = get_from_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix),received_prefix);
        if (test_neighbour_of_prefix != NULL)
        {
            ...
            while(my_temp != NULL)
            {
               //zlog_debug("this is the host of the peer %s i will send grobal convergence", my_temp -> peer -> host);
                char saving_prefix_value5[50];
                strcpy(saving_prefix_value5 ,received_prefix );
                bgp_convergence_send (my_temp -> peer,rec_root_cause_event_id_from_fizzle, received_prefix);
                strcpy(received_prefix,saving_prefix_value5 );
                my_temp = my_temp -> next;
             }
        }
        else
        {
            zlog_debug("in sending global convergnce We have not sent prefix %s to any neighbor!!!",received_prefix);
            //print_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix));
        }
      }
      else
        zlog_debug (" We have already setted the convergence value of this Event ID  %s to true !!!",char_received_root_cause_event_id);
    }
      else
      {
        zlog_debug ("**************************** it seems my sent is empty BUT I am not the owner of event it, the owner_id is %s and my id is %ld    ***************",char_owner_id,peer->local_as);
    struct senders_of_a_prefix * the_senders_of_the_prefix = (struct senders_of_a_prefix *) malloc (sizeof(struct senders_of_a_prefix));

    the_senders_of_the_prefix = get_from_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix),received_prefix);
    if (the_senders_of_the_prefix != NULL)
    {
        print_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix));
        struct peer_list * my_temp = the_senders_of_the_prefix -> peer_list ;
        ...
        while(my_temp != NULL)
        {
            //zlog_debug("FIZZLE........................this is the host of the peer %s we have received this prefix %s from lets send a fizzle to it", my_temp -> peer -> host,received_prefix );
            
            char saving_prefix_value[50];
            //sending_prefix_value = passed_prefix;
            strcpy(saving_prefix_value , received_prefix);
            // zlog_debug(".........saved value of the prifix %s ..........",saving_prefix_value);
            // zlog_debug(".........before sending to send fizzle function the prifix is %s ..........",received_prefix);

            bgp_fizzle_send (my_temp -> peer,rec_root_cause_event_id_from_fizzle, rec_root_cause_event_owener_id,received_prefix,rec_root_cause_event_owener_id);
            //zlog_debug(".........before using the saved value of the prifix %s ..........",received_prefix);

            strcpy(received_prefix , saving_prefix_value);
            //zlog_debug(".........using the saved value of the prifix %s ..........",received_prefix);
            my_temp = my_temp -> next;
         }
    }
    else
    {
        zlog_debug (" We have not received prefix %s from any neighbor!!!", received_prefix);
    }
    ...
    {
      zlog_debug (".............*************    sent is not empty  and I have to wait till I receive a fizzle from all of the below peers   *************......");
      ...
      //     zlog_debug("We are going to send a FIZZLE message to %s for prefix %s with time stamp %s and event id %s and E owner is %ld ",temp_cause->neighbour->host,temp_cause -> prefix_str,temp_cause->received_timestamp,char_received_root_cause_event_id,rec_root_cause_event_owener_id);
...
bgp_update_receive (struct peer *peer, bgp_size_t size)
{
  ...
  rec_root_cause_event_id = stream_getl (s);
  insert_in_converged(&(a_peer_for_maintating_head_of_data_structure -> converged), rec_root_cause_event_id);
...
//     zlog_debug ("this is the root_cause_event_id");
 ...
 //    the_senders_of_the_prefix = get_from_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix),);
      

 //    if (the_senders_of_the_prefix != NULL)
 //    {

 //        zlog_debug (" Before adding to the list of senders of the prefix");
 //        print_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix));
        
 //        remove_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix), "1.2.0.0");
 //        zlog_debug (" after removing to the list and neighbors of the prefix");
 //        print_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix));
 ...
 //        while(my_temp != NULL)
 //        {
 //            zlog_debug("this is the host of the peer %s we have received this prefix %s from", my_temp -> peer -> host,"1.2.0.0" );
 //            //bgp_convergence_send (my_temp -> peer,rec_root_cause_event_id, received_prefix);
 //            add_to_peer_list(&(test_appending_peer_list), my_temp -> peer);
 //            my_temp = my_temp -> next;
 //         }

 //        zlog_debug("Lets add this new peer %s to the list too as a peer we are receiving prefix %s from it",peer->host,"1.2.0.0");
 //        add_to_peer_list(&(test_appending_peer_list),  peer);
 //        zlog_debug("We added to the list");
 //        add_to_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix), "1.2.0.0", test_appending_peer_list);
 //        zlog_debug("We added the list to the senders of the prefix data structure");
 //        print_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix));
 //    }
 //    else
 //    {
 //        zlog_debug (" We have not received prefix %s from any neighbor!!!","1.2.0.0");

 //        struct peer_list* test_appending_peer_list2 = NULL;

 //        add_to_peer_list(&(test_appending_peer_list2),  peer);
 //        zlog_debug("We added just this one to the list");
 //        add_to_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix), "1.2.0.0", test_appending_peer_list2);
 //        print_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix));
...
bgp_read (struct thread *thread)
{
  ...
  //zlog_debug ("%s the first size is ", result32 );
   ...
   // zlog_debug ("We got error here " );
    ...
    && type != BGP_MSG_EVENT_DATA
    && type != BGP_MSG_FIB_ENTRY)
	{
	  if (BGP_DEBUG (normal, NORMAL))
	    plog_debug (peer->log,
 ...
 //zlog_debug ("%s this is the size int  of received packet ", result );
  ...
  switch (type) 
    ...
    case BGP_MSG_FIB_ENTRY:
    zlog_debug ("%s The type of received packet is new BGP_MSG_FIB_ENTRY, let's pars it", "horraaaaaaa........");
      bgp_new_FIB_entry_receive (peer, size);
      break;

    case BGP_MSG_EVENT_DATA:
      zlog_debug ("%s The type of received packet is BGP_MSG_EVENT_DATA, let's pars it", "horraaaaaaa........");
      bgp_event_data_receive (peer, size);
      break;
    case BGP_MSG_NOTIFY:
