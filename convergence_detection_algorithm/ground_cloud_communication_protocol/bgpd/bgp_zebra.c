...
bgp_zebra_announce (struct prefix *p, struct bgp_info *info, struct bgp *bgp, safi_t safi)
{
  ...
  //zlog_debug ("we are in bgp_zebra_announce for prefix %s ",inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])));
  ...
  // test_neighbour_of_prefix = get_from_neighbours_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> neighbours_of_prefix),inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])));
    
  //   if (test_neighbour_of_prefix == NULL)
  //   {
  //        zlog_debug (" We have not sent prefix %s to any neighbor!!!",inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])));
  ...
  //   the_senders_of_the_prefix = get_from_senders_of_a_prefix(&(a_peer_for_maintating_head_of_data_structure -> senders_of_prefix),inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])));
  ...
  //             res_rec_pref = get_from_received_prefix(&(a_peer_for_maintating_head_of_data_structure -> received_prefix),inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])), my_temp->peer);

  //           zlog_debug("FIZZLE........................this is the host of the peer %s we have received this prefix %s from lets send a fizzle to it", my_temp -> peer -> host,inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])) );
  //           bgp_fizzle_send (my_temp -> peer,res_rec_pref-> event_id, res_rec_pref->received_E_owner_id ,inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])),res_rec_pref->received_E_owner_id);
  //           my_temp = my_temp -> next;
  //        }
  //   }
  //   else
  //   {
  //       zlog_debug (" We have not received prefix %s from any neighbor!!!", inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])));
  //   }

  //   }
  //   else
  //       zlog_debug (" We have sent this prefix %s to some of the neighbors, so when we got the fizzle from them, we will send to senders!!!", inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])));
