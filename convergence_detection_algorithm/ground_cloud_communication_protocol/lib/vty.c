...
vty_new_init (int vty_sock)
{
  ...
  zlog_debug ("******************************************************************* this is vty_sock  %ld ", vty_sock);
...
vty_create (int vty_sock, union sockunion *su)
{
  ...
  zlog_debug ("******2**************!!!!!!**************************** this is vty socket value %ld", vty_sock);
...
vty_accept (struct thread *thread)
{
  ...
  zlog_debug ("********************!!!!!!**************************** this is vty socket value %ld", vty_sock);
...
vty_event (enum event event, int sock, struct vty *vty)
{
  ...
  switch (event)
    {
    case VTY_SERV:
      zlog_debug (" this is thread_add_read for vty ");
...
#ifdef VTYSH
    case VTYSH_SERV:
      zlog_debug (" this is thread_add_read again for vty ");
    ...
    case VTYSH_READ:
      ...
      break;
    case VTYSH_WRITE:
          zlog_debug (" this is thread_add_write again for vty ");

      vty->t_write = thread_add_write (vty_master, vtysh_write, vty, sock);
      break;
#endif /* VTYSH */
    case VTY_READ:
      zlog_debug (" this is thread_add_read again 3 for vty ");
